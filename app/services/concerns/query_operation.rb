...
module QueryOperation
  ...
  def compile_query(*parts, join_with: " ", prefix: false)
    compiled = parts.flatten.compact_blank.join(join_with).strip
  ...
  # Join components of a query with ` OR `.
  #
  # @param [<String>] parts
  # @param [Boolean] prefix
  # @return [String]
  def compile_or(*parts, prefix: false)
    compile_query(*parts, join_with: " OR ", prefix:)
  end
  ...
  def with_quoted_id_for(model, template)
    return "" if model.blank? || model.new_record? || model.destroyed?
